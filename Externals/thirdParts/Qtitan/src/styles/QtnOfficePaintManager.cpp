...
bool OfficePaintManager::drawHeaderSection(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    // 不明白为什么不画headerSection的左边线和上边线
    // 此行画上边线
    p->fillRect(QRect(QPoint(opt->rect.left(), opt->rect.top()), QSize(opt->rect.width(), 1)), d.m_clrColumnSeparator);

    // 此处画第一个section的左边线
    if (0 == opt->rect.left())
    {
        p->fillRect(QRect(QPoint(opt->rect.left(), opt->rect.top()), QSize(1, opt->rect.height())), d.m_clrColumnSeparator);
    }
...
void OfficePaintManager2013::modifyColors()
{
    ...
    if (!(d.m_clrTabSelectedText.isValid()))
      d.m_clrTabSelectedText = m_clrAccentText;
    if (!(d.m_clrTabHighlightText.isValid()))
      d.m_clrTabHighlightText = m_clrAccentText;

    d.m_clrHighlight = d.m_clrHighlight.isValid() ?
                d.m_clrHighlight : m_clrAccentLight;
    m_clrHighlightDn = d.m_clrHighlightPushed.isValid()?
                d.m_clrHighlightPushed : DrawHelpers::colorMakeDarker(m_clrAccentLight.rgb(), .14);
    m_clrHighlightChecked = d.m_clrHighlightChecked.isValid() ?
                d.m_clrHighlightChecked : DrawHelpers::colorMakeDarker(m_clrAccentLight.rgb(), .05);
