...
void GroupDefaultLayout::updateGeomArray_() const
{
    ...
    if(rg->style()->pixelMetric((QStyle::PixelMetric)RibbonStyle::PM_RibbonUserControlRowSpacing, 0, rg))
        baseHeight = resultHeight / rowCount;
...
GroupItem* GroupDefaultLayout::createItem(QAction* action)
{
    ...
    else if (action->isSeparator()) 
    {
        ...
        sep->setVisible(action->isVisible());
        hasSeparator = true;
        widget = sep;
    }
...
void RibbonGroupPrivate::hidePopupMode(QAction* action)
{
    Q_UNUSED(action);
    QTN_P(RibbonGroup)
    //修改在group压缩成按钮后没有显示菜单的情况下action被触发导致的group消失的问题
    if(m_ribbonDefaultGroupButton)
    {
        //如果再action执行过程中出现了模态窗体，group的事件循环会被打断导致运行到此处的时候group已经不可见了
        if (GroupDefaultLayout* gLayout = qobject_cast<GroupDefaultLayout*>(p.layout()))
            if (QWidget * w = this->getWidget(gLayout->indexOf(action)))
            {
                w->setAttribute(Qt::WA_UnderMouse, false);
                //取消下次显示时的褪色动画
                w->setProperty("_q_stylestate", 0);
            }

        if(m_ribbonDefaultGroupButton->isShowPopup())
            p.setVisible(false);
    }
}
...
/*!
  \brief Adds \a action before other action that specified in \a before argument. In result action's widget will be placed before widget that is associated with action \a before. 
*/ 
QAction* RibbonGroup::insertAction(QAction* before, QAction* action, Qt::ToolButtonStyle style, QMenu* menu, QToolButton::ToolButtonPopupMode mode)
{
    ...
    Q_ASSERT(indexAction != -1);

    QLayoutItem* item = layout()->itemAt(indexAction);
    if ( QToolButton* widgetAction = qobject_cast<QToolButton*>(item ? item->widget() : Q_NULL) )
    {
        widgetAction->setToolButtonStyle(style);
...
QAction *RibbonGroup::addBeginning()
{
    QAction *action = new QAction(this);
