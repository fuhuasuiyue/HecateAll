...
void RibbonPage::moveGroup(RibbonGroup *fromGroup, RibbonGroup *preGroup)
{
    QTN_D(RibbonPage);
    if (NULL == fromGroup)
        return;
    if (NULL == preGroup)
    {
        d.m_listGroups.move(d.getIndexGroup(fromGroup), 0);
    }
    else
    {
        int toGroupIndex = d.getIndexGroup(preGroup) + 1;
        if (toGroupIndex >= d.m_listGroups.count())
        {
            toGroupIndex = d.m_listGroups.count() - 1;
        }
        d.m_listGroups.move(d.getIndexGroup(fromGroup), toGroupIndex);
    }
}
...
void RibbonPage::beginUpdate()
{
    QTN_D(RibbonPage);
    d.m_inUpdate = true;
}

void RibbonPage::endUpdate()
{
    QTN_D(RibbonPage);
    d.m_inUpdate = false;
...
bool RibbonPage::event(QEvent* event)
{
    ...
    switch(event->type())
    {
        case QEvent::LayoutRequest :
            {
                //+8 2015.10.09 by xiangrl
                //如果action状态改变，非当前page的重新布局会导致布局的变化从而引发问题
                if(RibbonBar * rb = qobject_cast<RibbonBar*>(parentWidget()))
                {
                    if(rb->getPage(rb->currentIndexPage()) != this)
                    {
                        event->ignore();
                        break;
                    }
