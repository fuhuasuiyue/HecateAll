...
QSize RibbonSystemPopupBar::sizeHint() const
{
    ...
    int hei = 0;
    if (d.m_pageList.count() > 0 && qobject_cast<RibbonPageSystemRecentFileList*>(d.m_pageList[0]))
    {
        hei = qobject_cast<RibbonPageSystemRecentFileList*>(d.m_pageList[0])->recentFileHeight() + 40 + 12;
    }
    sz.setHeight(sz.height() > hei ? sz.height() : hei);
...
//            QSize(1, sz.height() - d.m_rcBorders.top() - d.m_rcBorders.bottom()/2-2)), fillColor);
        ...
        p.fillRect(QRect(QPoint(sz.width() + d.m_rcBorders.left() - hMargin, d.m_rcBorders.top()),
            QSize(1, sizeHint().height() - d.m_rcBorders.bottom() - d.m_rcBorders.top())), fillColor);
...
void RibbonSystemPopupBar::resizeEvent(QResizeEvent* event)
{
    ...
    for (int i = 0; i < d.m_pageList.size(); i++)
    {
        ...
        {
            QSize sz = QMenu::sizeHint();
            sz.setHeight(rect().size().height());
            int hMargin = style()->pixelMetric(QStyle::PM_MenuHMargin, Q_NULL, this)*2;
            QRect rc(QPoint(sz.width() + d.m_rcBorders.left() - (hMargin-1), d.m_rcBorders.top()), 
                QSize(page->sizeHint().width(), sz.height() - d.m_rcBorders.bottom() - d.m_rcBorders.top()/2+2 - 12));
            page->setGeometry(rc);
            break;
        }
    }
...
void RibbonPageSystemRecentFileListPrivate::updateActionRects() const
{
    ...
    const_cast<int &>(static_cast<const RibbonPageSystemRecentFileListPrivate&>(*this).m_actionHeight) = y;
    //m_actionHeight = y;
...
int RibbonPageSystemRecentFileList::recentFileHeight() const
{
    QTN_D(const RibbonPageSystemRecentFileList);
    return d.m_actionHeight;
}
