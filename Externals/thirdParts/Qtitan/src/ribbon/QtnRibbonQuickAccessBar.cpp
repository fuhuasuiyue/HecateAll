...
void RibbonQuickAccessBarPrivate::deleteAction(QAction *action)
{
    ...
    m_menu->clear();
    QuickAccessAction *pAction = findQuickAccessAction(action);
    if (pAction)
    {
        m_actionList.removeOne(pAction);
        m_customizeGroupAction->removeAction(pAction);
    }
    for (int i = 0; i < oActions.count(); ++i)
    {
        QuickAccessAction *actionWrapper = dynamic_cast<QuickAccessAction*>(oActions[i]);
